...
class Trainer:
    def __init__(self, opt, model, optimizer, lr_schedule, train_data_loader, valid_data_loader=None, start_epoch=0):
        ...
        self.criterion = nn.BCELoss()     # binary_cross_entropy
    ...
    def train(self):
        ...
        for epoch in range(self.opt.max_epoch):   # max_epoch=300
            progressbar = tqdm(self.train_data_loader)
            d_loss = AverageMeter()
            g_loss = AverageMeter()
            c_loss = AverageMeter()
            s_loss = AverageMeter()
            for ii, imgs in enumerate(progressbar):
                ...
                if (ii + 1) % self.opt.d_every == 0:   # d_every=1
                    # train discriminator
                    self.netd.train()
                ...
                if (ii + 1) % self.opt.g_every == 0:   # g_every=1
                    # train generator
                    ...
                    losses = error_g + self.opt.contrast_loss_weight * error_c  # contrast_loss_weight=1
                    losses.backward()
                ...
                progressbar.set_description(
                    'Epoch: {}. Step: {}. Discriminator loss: {:.5f}. Generator loss: {:.5f}. '
                    'Contrast loss: {:.5f}. Segmentation loss: {:.5f}'.format(epoch, ii, d_loss.getavg(),
                                                                              g_loss.getavg(), c_loss.getavg(),
                                                                              s_loss.getavg()))
